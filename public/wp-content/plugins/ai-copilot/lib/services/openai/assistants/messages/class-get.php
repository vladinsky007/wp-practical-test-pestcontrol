...
namespace QuadLayers\AICP\Services\OpenAI\Assistants\Messages;
...
/**
 * API_Fetch_Assistant_Message_OpenAi Class extends Base
 */
class Get extends Base {
	...
	public function get_url( $args = null ) {
		$params = array(
			'before' => $args['message_openai_before'] ? $args['message_openai_before'] : '',
			'limit'  => $args['limit'] ? $args['limit'] : 1,
			'order'  => 'desc',
		);
		return $this->fetch_url . '/threads/' . $args['thread_openai_id'] . '/messages?' . http_build_query( $params );
	}

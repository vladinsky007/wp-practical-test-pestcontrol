...
class Follows {
	...
	public function add_templates( $templates ) {
		$templates['followers'] = [
			...
			'callback_for_page'     => function() {
				require wpftpl( 'profile-followers.php' );
			},
		];
		$templates['following'] = [
			'type'                  => 'callback',
			...
			'add_in_member_buttons' => 0,
			'callback_for_page'     => function() {
				require wpftpl( 'profile-following.php' );
			},
		];
	...
	public function decode( $follow ) {
		...
		$follow['name']     = trim( strip_tags( (string) $follow['name'] ) );
		$follow['email']    = sanitize_email( $follow['email'] );
		if( ! ( $follow['itemtype'] = trim( strip_tags( (string) $follow['itemtype'] ) ) ) ) $follow['itemtype'] = 'user';
		if( ! ( $follow['confirmkey'] = trim( (string) $follow['confirmkey'] ) ) ) $follow['confirmkey'] = $this->get_confirm_key();

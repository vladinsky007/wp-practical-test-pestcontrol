...
<div id="wpf-admin-wrap" class="wrap">
	...
	<?php if( in_array( $action, [ 'add', 'edit' ] ) ) : ?>
        ...
        <script type="text/javascript">
            if (typeof wpforo_setAttr !== 'function') {
                // The function exists
                /**
                 *
                ...
                $('#forum_layout').on('click', '.wpf-fl-box:not(.wpf-fl-active) h4', function () {
                    if ($('#use_us_cat').is(':checked')) {
                        ...
                        if (wrap) {
                            ...
                            if (layout) {
                                const opt = $('#layout option[value="' + layout + '"]');
                                wpforo_setAttr( opt, 'selected', true );
                                ...
                                <div class="inside">
                                    <div class="wpf-forum-additional-options">
                                        ...
                                        <div class="wpforo-style-field">
                                            <input class="wpforo-color-field" name="forum[color]" type="text" value="<?php wpfo( strtoupper( (string) $color ) ); ?>" title="<?php wpfo( strtoupper( (string) $color ) ); ?>"/>
                                        </div>
                                    ...
                                    <div class="wpf-fl-wrap">
										...
										if( ! empty( $layouts ) ): foreach( $layouts as $layout ) : ?>
                                            <div id="wpfl-<?php echo intval( $layout['id'] ) ?>" data-layout="<?php echo intval( $layout['id'] ) ?>" class="wpf-fl-box <?php if( wpfkey( $data, 'is_cat' ) && $layout['id'] == $data['layout'] ) echo 'wpf-fl-active' ?>">
                                                ...
                                                <h4><?php echo esc_html( $layout['name'] ) ?><!-- <input type="radio" name="forum[layout]" value="<?php echo esc_attr( trim( (string) $layout['id'] ) ) ?>" <?php echo( $layout['id'] == $data['layout'] ? ' checked="checked" ' : '' ); ?> /> --></h4>
                                            </div>
